...
import { pgTable, varchar, smallint, timestamp, jsonb, integer, boolean, index } from "drizzle-orm/pg-core";
 ...
 */
export const createTable = (name: string, schema: Record<string, any>) => pgTable(`tdz_${name}`, schema);
...
export const classes = pgTable(
  "classes",
  {
    ...
    testNumber: smallint("test_number").default(1),
    teacherId: varchar("teacher_id", { length: 36 }).references(() => teachers.id),
...
export const studentAnswers = pgTable("student_answers", {
  ...
  questionId: varchar("question_id", { length: 36 }).references(() => questions.id),
  answer: jsonb("answer"),
...
export const studentModuleScores = pgTable(
  "student_module_scores",
  {
    ...
    moduleId: varchar("module_id", { length: 36 }).references(() => modules.id).notNull(),
...
export const questionStats = pgTable("question_stats", {
  ...
  classId: varchar("class_id", { length: 36 }).references(() => classes.id).notNull(),
  testNumber: smallint("test_number").notNull(),
  questionId: varchar("question_id", { length: 36 }).references(() => questions.id).notNull(),
